 ...
 * the interface ParkGUI
 * 
 * @author Mouna Kacem, sanghyun lee
 */
public class Animal implements ParkGUI {
	...
	protected JunglePark processing; // PApplet object that represents the
										// display window
	protected PImage image; // animal's image

	private float[] position; // animal's position in the display window
								// Usage: position[0: x-coordinate, or 1:
								// y-coordinate]
	private boolean isDragging; // indicates whether the animal is being dragged
								// or not
	 ...
	 * @param imageFileName
	 *            filename of the animal image
	 */
	public Animal(JunglePark processing, float positionX, float positionY, String imageFileName) {

		// Set Animal drawing parameters
		this.processing = processing; // set the PApplet Object where the animal
										// will be drawn
		this.position = new float[] { positionX, positionY }; // sets the
																// position of
																// the animal
																// object
		this.image = processing.loadImage(imageFileName);
		isDragging = false;
		// the animal is not dragging
	}
	 ...
	 * mouse position if the tiger is being dragged (i.e. if its isDragging
	 * field is set to true).
	 */
	@Override
	public void draw() {
		// if the tiger is dragging, set its position to the mouse position with
		// respect to the display
		// window (processing) dimension
		if (this.isDragging) {
			if (this.processing.mouseX < 0) // mouse outside the screen
				this.position[0] = 0;
			else if (this.processing.mouseX > this.processing.width) // mouse
																		// outside
																		// the
																		// screen
			...
			if (this.processing.mouseY < 0) // mouse outside the screen
				this.position[1] = 0;
			else if (this.processing.mouseY > this.processing.height) // mouse
																		// outside
																		// the
																		// screen
		...
		// display label
		displayLabel();
		this.action(); // call to action() method 
	 ...
	 * 
	 * @param tiger
	 *            reference to a given Tiger object
	...
	public boolean isMouseOver() {
		 ...
		 * @param tiger
		 *            reference to a given Tiger object
		 * @return true if the mouse is over the given tiger object, false
		 *         otherwise
		 * 
		 */
	...
	/**
	 * Computes the euclidean distance between the current animal and another
	 * one
	 * 
	 * @param otherAnimal
	 *            reference to another animal
	...
	/**
	 * returns TRUE if otherAnimal is located within range distance around the
